 ...
 * reads followed by up to {@code writeBatch} writes. This groups reads
...
public class ReadWriteCallQueue<E extends Schedulable> extends AbstractQueue<E>
  ...
  private static final Log LOG =
      LogFactory.getLog(ReadWriteCallQueue.class);

  public static final String READ_BATCH_KEY = "readwrite.read.batch";
  public static final String WRITE_BATCH_KEY = "readwrite.write.batch";

  public static final int READ_BATCH_DEFAULT = 128;
  public static final int WRITE_BATCH_DEFAULT = 64;
  ...
  /** Cumulative count of read calls inserted into {@link #readQueue}. */
  private final AtomicLong readInsertCount = new AtomicLong(0);
  /** Cumulative count of write calls inserted into {@link #writeQueue}. */
  private final AtomicLong writeInsertCount = new AtomicLong(0);
  ...
  public ReadWriteCallQueue(int priorityLevels, int capacity, String ns,
    ...
    this.writeBatch = Math.max(1,
        conf.getInt(ns + "." + WRITE_BATCH_KEY, WRITE_BATCH_DEFAULT));

    this.readsRemaining = readBatch;
    this.writesRemaining = writeBatch;
    this.readPhaseCount.incrementAndGet();

    LOG.warn("ReadWriteCallQueue constructed: namespace=" + ns
        + ", capacity=" + capacity
        + ", readBatch=" + readBatch + " (" + ns + "." + READ_BATCH_KEY + ")"
        + ", writeBatch=" + writeBatch + " (" + ns + "." + WRITE_BATCH_KEY + ")"
        + ", slots=" + slots.availablePermits()
  ...
  private void insert(E e) {
    if (e.isWrite()) {
      writeQueue.add(e);
      writeInsertCount.incrementAndGet();
    } else {
      readQueue.add(e);
      readInsertCount.incrementAndGet();
    }
  }
  ...
  public long getReadInsertCount() {
    return readInsertCount.get();
  }

  /** Cumulative number of write calls inserted into the write sub-queue. */
  public long getWriteInsertCount() {
    return writeInsertCount.get();
  }

  /**
   * Number of free capacity slots. Equal to {@link #remainingCapacity()}.
   * This is the {@code slots} statistic the user asked to expose.
   */
  ...
  private static final class MetricsProxy implements ReadWriteCallQueueMXBean {
    ...
    // Weak ref for delegate, so we don't retain it forever if it can be GC'd
    ...
    public long getWriteInsertCount() {
      ...
      return obj.getWriteInsertCount();
    }
